...
class SingleViewOpt(nn.Module):
    ...
    def forward(self, carm, R_init, t_init, kernel=1.0, iters_per_scale=100, patience=15, verbose=True):
        """
        Parameters
        ----------
        carm: Input fluoroscopy / DRR.
        R_init: Initial rotation matrix (B,3,3).
        t_init: Initial translation (B,3).

        Returns:
        ----------
        best_R, best_t, best_similarity, history
        """
        ...
        for scale in self.scales:
            ...
            for step in range(iters_per_scale):
                ...
                # Render DRR
                proj = self.render_drr(R_current, t_current, drr=drr,img_size=img_size)

                if isinstance(kernel_s, float):
                    proj = proj * kernel_s
                else:
                    proj = proj * kernel_s
            ...
            # --------------------------------------------------
            # Use the optimized pose from this scale as the base pose for the next (finer) scale.
            # --------------------------------------------------
            R_base, t_base = clone_pose(best_R.detach(), best_t.detach())

            # Reset optimization variables so each pyramid level
            # estimates only a local refinement.
            with torch.no_grad():
                delta_rot.zero_()
                delta_trans.zero_()

        return best_R, best_t, best_similarity, history
...
class ViewData:
    ...
    R_delta_1i : Tensor
        (1, 3, 3) fixed relative rotation from the reference view
        (view 0) to this view.

        For the reference view this should be the identity matrix.

    t_delta_1i : Tensor
        (1, 3) fixed relative translation from the reference view
        (view 0) to this view.
...
class MultiViewObjective:
    ...
    def __init__(self, views: List[ViewData], renderer_factory: Callable[[int], DRR], criterion: nn.Module, resize_fn: Callable, normalize_fn: Callable):
        ...
        ----------
        views
            List of all views. views[0] is the reference.

        renderer_factory
            Callable taking img_size and returning a DRR renderer.

        criterion
            Similarity metric (mGNCCLoss).

        resize_fn
            SingleViewOpt.resize.

        normalize_fn
            SingleViewOpt.normalize.
        """
    ...
    def _render_batch(self, renderer: DRR, R: torch.Tensor, t: torch.Tensor, img_size: int) -> torch.Tensor:
        ...
        img_size
            Current pyramid level resolution.
    ...
    def __call__(self, delta_rot: torch.Tensor, delta_trans: torch.Tensor, img_size: int) -> Tuple[torch.Tensor, List[float]]:
        ...
        ----------
        delta_rot
            (1,3) axis-angle update applied to the reference pose.

        delta_trans
            (1,3) translation update (mm) applied to the reference pose.
        ...
        # ----------------------------------------------------------
        # Step 1: Update reference pose
        # ----------------------------------------------------------

        R1_cur, t1_cur = apply_delta(self.R1_init, self.t1_init, delta_rot, delta_trans)

        # ----------------------------------------------------------
        # Step 2: Compute all dependent poses simultaneously
        # ----------------------------------------------------------
...
class MultiViewOptimizer(SingleViewOpt):
    ...
    def _check_view_consistency(self, views: List[ViewData]) -> None:
        ...
        for idx, view in enumerate(views[1:], start=1):
            ...
            if rot_err > torch.deg2rad(5.0) or trans_err > 10.0:
                print(
    ...
    def forward_multiview(self,
                          views: List[ViewData],
                          iters_per_scale: int = 100,
                          patience: int = 15,
                          verbose: bool = True) -> Tuple[torch.Tensor, torch.Tensor, float, dict]:
        """
        ...
        ----------
        views
            List of ViewData objects. views[0] is the reference view.

        Returns
        -------
        best_R
            Optimized reference rotation matrix.

        best_t
            Optimized reference translation.

        best_loss
            Lowest joint loss achieved during optimization.
        ...
        delta_rot = nn.Parameter(
            torch.zeros((1, 3), device=self.device),
            requires_grad=True,
        )

        delta_trans = nn.Parameter(
            torch.zeros((1, 3), device=self.device),
            requires_grad=True,
        )
        ...
        for scale in self.scales:
            ...
            # img_size = config.IMAGE_SIZE // scale

            objective.set_reference_pose(best_R, best_t)

            delta_rot.data.zero_()
            ...
            scale_best_R, scale_best_t, scale_best_loss = self._optimize_scale(
                objective=objective,
                optimizer=optimizer,
                delta_rot=delta_rot,
        ...
        return best_R, best_t, best_loss, history

    def _optimize_scale(self,
                        objective: MultiViewObjective,
                        ...
                        iters_per_scale: int,
                        history: dict,
                        verbose: bool = True) -> Tuple[torch.Tensor, torch.Tensor, float]:
        """
        ...
        delta_rot
            Learnable axis-angle update.

        delta_trans
            Learnable translation update.
        ...
        -------
        best_R
            Best reference rotation found for this scale.

        best_t
            Best reference translation found for this scale.

        best_loss
            Lowest joint loss achieved during this scale.
        """
        ...
        for step in range(iters_per_scale):
            ...
            joint_loss, per_view_ncc = objective(
                delta_rot,
                delta_trans,
                img_size,
            )
            ...
            # --------------------------------------------------
            # Compute current reference pose after the update
            # --------------------------------------------------
